...
//main program
void correlate_EASIROC_HBU(int runnum){
  TString matchout;
  TString hbufinname;
  TString hbufout;
  ...
  TFile* f[2];
  for (int i = 0; i < 2; i++) {
    easirocfinname[i].Form("./data/hodoscope%dRaw_00%d.raw",i+1,runnum);
    easirocfout[i].Form("./rootfile/easiroc_%d_%d.root",i+1,runnum);
    f[i]= new TFile(easirocfout[i].Data(), "RECREATE");
    loadeasiroc(easirocfinname[i],f[i],i);
    convert(easirocfout[i]);

  }
  mergeeasirocfout.Form("./rootfile/merge_easiroc_%d.root",runnum);
